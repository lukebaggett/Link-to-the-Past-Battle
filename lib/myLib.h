...
/* Mode 3 */
extern unsigned char * videoBuffer;               // 0x6000000
...
typedef struct {
  ...
  size, // width*height/2
  dir, // Direction of frame animation
  index, // Index of current animation frame
  count, // Number of animation frames
  idle, // Index of idle frame
  counter;
} FRAME;

typedef struct {
  int idle, // Sprite is in idle state
  hit, // Sprite is in hit state
  attacking, // Sprite is attacking
  dying, // Sprite is dying
...
typedef struct {
  ...
  int ID;
  POINT pos;
  POINT dv; // Delta V, v in each direction
  POINT spawn; // Spawn Position
  FRAME move;
  FRAME attack;
  STATE state;
  POINT hit;
...
void drawHollowRectangle(int x, int y, int width, int height, unsigned short color);
